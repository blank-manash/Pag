...
app.get("/:day",function(req,res){
  const day = lo.kebabCase(req.params.day);
  //console.log(day);
  let flag = 0;
  let found = [];
  posts.forEach(function(elem)
  {
    if(lo.kebabCase(elem.title) == day){
      flag = 1;
      found = elem;
    }
  });
  if(flag) res.render("post",{heading : found.title, blogpost : found.content});
  else res.redirect("/");
});
